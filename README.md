...
- **근무 중인 하루의 응답 모양은 아직 검증하지 못했습니다.** 끝 타임스탬프가 없는 `WORK` 블록으로 온다고 가정했습니다. 만약 진행 중에도 종료 시각이 채워져 오면 오늘이 분모에서 빠져 하루 필요시간이 실제보다 크게(안전한 방향으로) 나옵니다. 퇴근 전에 아무 블록도 오지 않는 경우라면 가정과 무관하게 정상 동작합니다.
